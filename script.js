...
viewCertificates?.addEventListener("click", () => {
  ...
  certificateCards.forEach(card => {
    ...
    item.innerHTML = `
      <img class="certificate-gallery-image" src="${escapeHtml(file)}" alt="${escapeHtml(title)}" loading="lazy">
      <span class="certificate-gallery-title">${escapeHtml(title)}</span>
...
/* =========================================================
   HERO SOCIAL CIRCLE CLICK FEEDBACK
   Does not alter navigation or link behavior.
========================================================= */
document.querySelectorAll(".hero .social").forEach(icon => {
  icon.addEventListener("click", () => {
    icon.classList.remove("social-pulse");
    void icon.offsetWidth;
    icon.classList.add("social-pulse");
    setTimeout(() => icon.classList.remove("social-pulse"), 600);
  });
});

/* =========================================================
   PERFORMANCE GUARD
   Keep the existing interactions, but avoid desktop-only
   pointer/magnetic work on touch devices.
========================================================= */
